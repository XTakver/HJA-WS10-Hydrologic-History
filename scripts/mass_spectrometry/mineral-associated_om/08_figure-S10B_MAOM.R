# ------------------------------------------------------------------------------
# 08_figure-S10B_MAOM.R
#
# MAOM condensed heatmap (FTICR-MS) -- post-QAQC + 2/3 consensus
# ------------------------------------------------------------------------------
# Purpose:
#   - Read replicate-aggregated MAOM presence/absence (post-MAD filtering + 2/3 rule)
...
maom_pa <- read_csv(file.path(DD, '2023_FTICR-MS_mineral-associated_om_aggregated.csv'))

# Per-formula properties table (includes compound class used for column splits)
maom_chemProp <- read_csv(file.path(DD, '2023_FTICR-MS_mineral-associated_om_ChemProperties_aggregated.csv'))
...
maom_combined <- maom_pa |>
  left_join(maom_chemProp |> select(molecular_formula, class),
...
CairoPDF(file.path(PD, 'Fig.S10B_MAOM_Heatmap.pdf'), width = 4.5, height = 2)
draw(
  plt.heatmap,
  use_raster = TRUE,
  raster_quality = 2
