# -----------------------------------------------------------------------------#
# 05_rarefied_NMDS_MAOM.R
#
# Purpose
#   Pulls in MAD-filtered MAOM presence/absence data and performs rarefied NMDS.
...
#   - 2023_FTICR-MS_NMDS_Scores_MAOM.csv
#   - 2023_FTICR-MS_NMDS_SampleProp_Scores_MAOM.csv
#   - mineral-associated_om_Jaccard_avgdist_rare2195_it750.rds
#
# Key settings
#   - Distance: Jaccard (binary)
#   - Rarefaction: sample = 2195, iterations = 750
#   - NMDS: k = 4, trymax = 100
...
hcl_sampleProp <- read_csv(file.path(DD, '2023_FTICR-MS_mineral-associated_om_Sample_Properties_MADfiltered.csv'))
...
min(hcl_summary$Total.Compounds)
max(hcl_summary$Total.Compounds)

# Minimum peaks detected: 2,195
# Maximum peaks detected: 11,250
...
hcl_nmds_scores <- as.data.frame(scores(nmds_result)) |>
...
nmds_hcl_sampleProp <- hcl_sampleProp |>
...
fit_mol_prop <- envfit(nmds_result, nmds_hcl_sampleProp, permutations = 999, choices = 1:4)

# Extract fitted vector coordinates and p-values for plotting/annotation
hcl_sampleProp_nmdsScores <- as.data.frame(scores(fit_mol_prop, 'vectors')) |>
...
write_csv(hcl_nmds_scores, file.path(OD, '2023_FTICR-MS_NMDS_Scores_MAOM.csv'))
write_csv(hcl_sampleProp_nmdsScores, file.path(OD, '2023_FTICR-MS_NMDS_SampleProp_Scores_MAOM.csv'))

# Save rarefied dissimilarity matrix as an intermediate object for reproducibility
saveRDS(dissimilarity_matrix, file.path(OD, 'mineral-associated_om_Jaccard_avgdist_rare2195_it750.rds'),
        compress = 'xz')
