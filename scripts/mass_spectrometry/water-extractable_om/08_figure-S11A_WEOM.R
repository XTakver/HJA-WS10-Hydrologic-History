# ------------------------------------------------------------------------------
# 08_figure-S11A_WEOM.R
#
# WEOM compound-class composition -- normalized barplot (unique formulas)
# ------------------------------------------------------------------------------
# Purpose:
#   - Import WEOM sample-average composition metrics computed from unique formulas
...
DD <- here('data', 'mass_spectrometry', 'processed', 'water-extractable_om', 'unique_formulas')
...
weom_sampleProp <- read_csv(file.path(DD, '2023_FTICR-MS_WEOM_SampleAvg_Unique.csv'))
...
weom_properties <- weom_sampleProp |>
...
df_weom_normalized <- weom_barplot |>
...
plt.weom.norm <-
  ggplot(df_weom_normalized, aes(x = Sample.ID, y = relative_comp, fill = Class)) +
...
CairoPDF(file.path(PD, 'Fig.S11A_WEOM_CompClass.pdf'), width = 4, height = 6)
print(plt.weom.norm)
dev.off()
...
rm(weom_sampleProp, weom_properties, weom_barplot, df_weom_normalized,
   plt.weom.norm)
