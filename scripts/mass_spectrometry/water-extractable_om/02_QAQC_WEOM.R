...
#
# Purpose
#   QA/QC for WEOM (FTICR-MS): screens replicate outliers by per-sample
...
#
# Inputs
#   2023_MONet_FTICR-MS_WEOM_Intensities.csv
#   2023_MONet_FTICR-MS_WEOM_Presence-Absence.csv
#   2023_MONet_FTICR-MS_WEOM_Avg_Molecular-Properties.csv
#   2023_MONet_FTICR-MS_WEOM_Molecular-Properties.csv
#   (all written by 01_processing_WEOM.R)
#
# Outputs
#   Data (written to OD):
#     - 2023_FTICR-MS_WEOM_PA_MADfiltered.csv
#     - 2023_FTICR-MS_WEOM_Intensity_MADfiltered.csv
...
#     - preQAQC_WEOM_boxplot.pdf / preQAQC_WEOM_histogram.pdf
#     - WEOM_MAD_boxplot.pdf
#
# Notes
...
PD <- here('output', 'figures', 'mass_spectrometry', 'water-extractable_om', 'qaqc')
...
# Read WEOM inputs (presence/absence drives peak-count QAQC; other tables are filtered downstream)
df_intensities <- read_csv(file.path(DD, '2023_MONet_FTICR-MS_WEOM_Intensities.csv'))
df_PA <- read_csv(file.path(DD, '2023_MONet_FTICR-MS_WEOM_Presence-Absence.csv'))
df_avg_mol_prop <- read_csv(file.path(DD, '2023_MONet_FTICR-MS_WEOM_Avg_Molecular-Properties.csv'))
df_mol_prop <- read_csv(file.path(DD, '2023_MONet_FTICR-MS_WEOM_Molecular-Properties.csv'))
...
boxplt_all <- ggplot(df_peaks) +
  ...
  labs(y = 'Peak #', fill = 'Core Section', title = 'WEOM Outlier Boxplot w/ all data')

boxplt_all

CairoPDF(file.path(PD, 'preQAQC_WEOM_boxplot.pdf'), width = 6.5, height = 4)
print(boxplt_all)
dev.off()
...
plt_hist <- ggplot(df_hist) +
  ...
  labs(title = 'WEOM Outlier Distribution w/ all data')

plt_hist

CairoPDF(file.path(PD, 'preQAQC_WEOM_histogram.pdf'), width = 15, height = 8.5)
print(plt_hist)
dev.off()
...
CairoPDF(file.path(PD, 'WEOM_MAD_boxplot.pdf'), width = 15, height = 8.5)
print(boxplt_MAD)
dev.off()
...
write_csv(df_PA_MAD, file.path(OD, '2023_FTICR-MS_WEOM_PA_MADfiltered.csv'))
write_csv(df_intensity_MAD, file.path(OD, '2023_FTICR-MS_WEOM_Intensity_MADfiltered.csv'))
write_csv(df_AvgMol_MAD, file.path(OD, '2023_FTICR-MS_WEOM_Sample_Properties_MADfiltered.csv'))
write_csv(df_MolProp_MAD, file.path(OD, '2023_FTICR-MS_WEOM_ChemProperties_MADfiltered.csv'))
